...
public class InventoryMenuView {
    ...
    private Texture px;   // 1x1 white
    private final Texture leftArrow;
    private final Texture rightArrow;
    private float leftX, rightX, arrowsY, arrowSize;
    private Runnable onOpenSellMenu;
    public void setOnOpenSellMenu(Runnable r) { this.onOpenSellMenu = r; }
    ...
    public InventoryMenuView(Inventory inventory, Inventory fridge, GameController controller) {
        ...
        sellBinTex = new Texture("inventory/cook_button.png");
    ...
    public void render(SpriteBatch batch) {
        ...
        trashY    = panelY + 24;
        float rightEdge = trashX;
        if (controller.isPlayerInHouse()) {

            moveBtnW = 65; moveBtnH = 55;
            moveBtnX = rightEdge - moveBtnW - 8;
            moveBtnY = trashY - 5;
            rightEdge = moveBtnX - 8;  // فضای بعدی برای Sell Bin
        }

// Sell Bin همیشه هست: یک خونه سمت چپِ قبلی
        binX = rightEdge - binW;
        binY = trashY - 5;
...
// Trash
        ...
        batch.draw(slotBg, binX, binY, binW, binH);          // پس‌زمینه (اختیاری)
        batch.draw(sellBinTex, binX, binY, binW, binH);      // خود آیکون
        ...
        switch (tabIndex) {
            ...
            case 1: drawSkillsPage(batch, panelX, panelY, panelW, panelH); break;
    ...
    private void drawSkillsPage(SpriteBatch batch, float px, float py, float pw, float ph) {
        ...
        float x = px + 40, y = py + ph - 90;

        big.draw(batch, "Skills", x, y + 30);
        for (Skill t : Skill.values()) {
            SkillState s = player.getSkills().get(t);
            String line = t.name().charAt(0) + t.name().substring(1).toLowerCase()
                    + "  Lv." + s.level() + "   (" + s.xp() + "/" + s.xpToNext() + ")";
            small.draw(batch, line, x, y);

            // progress bar
            float barW = 220, barH = 10;
            float p = s.xpToNext() == 0 ? 0f : s.xp() / (float) s.xpToNext();

            // مستطیل ساده با slotBg یا با shapeRenderer
            Texture slot = this.slotBg;
            batch.draw(slot, x, y - 16, barW, barH);
            batch.draw(slot, x, y - 16, barW * p, barH);

            y -= 40;
        }

        small.draw(batch, "Max Energy: " + player.getMaxEnergy(), x, y - 10);
    }
    ...
    private void handleInput() {
        ...
        if (Gdx.input.isKeyJustPressed(Input.Keys.UP) && row > 0) {
            ...
            float mx = Gdx.input.getX();
            float my = Gdx.graphics.getHeight() - Gdx.input.getY();
            if (mx>=binX && mx<=binX+binW && my>=binY && my<=binY+binH) {
                // expose a callback:
                if (onOpenSellMenu != null) onOpenSellMenu.run();
            }
        }
